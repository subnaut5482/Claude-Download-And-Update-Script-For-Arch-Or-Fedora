...
Unofficial bash script to install and update **Claude Desktop** on Linux distributions not officially supported by Anthropic (e.g. Arch Linux, CachyOS, Manjaro and derivatives), without relying on third-party AUR packages.
...
- Tested on **KDE Plasma 6** (CachyOS/Arch). On other desktop environments, the application menu refresh may not work the same way.

## Requirements

- `curl`
- `binutils` (provides `ar` and `strings`, usually already present on Arch)
- `tar`
...
2. Detects the currently installed version, trying in order:
   ...
   - `dpkg` (if installed via apt)
   - `pacman` (if installed via an AUR package)
...
6. Updates the `.desktop` file and icons in the application menu, and rebuilds the KDE Plasma cache (`kbuildsycoca6`/`kbuildsycoca5`).
